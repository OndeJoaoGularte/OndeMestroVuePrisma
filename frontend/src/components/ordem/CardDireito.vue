<template>
  <v-card class="d-flex flex-column profile-card" variant="outlined">
    <v-card-title class="d-flex align-center justify-space-between py-2 bg-surface-light flex-0-0">
      ...
      <v-btn
        size="default"
        variant="tonal"
        :color="showInv ? 'indigo' : 'brown'"
        @click="showInv = !showInv"
      >
      ...
      <v-window v-model="showInv" class="h-100">
        <v-window-item :value="false" class="h-100">
          <div class="d-flex flex-column h-100">
                        ...
                        <template v-slot:default="{ expanded }">
                          <v-row no-gutters align="center">
                            <v-col cols="auto" class="mr-2">
                              <v-chip
                                size="x-small"
                                :color="getHabilidadeColor(hab.origem)"
                                label
                                >{{ hab.origem }}</v-chip
                              >
                            </v-col>
                            ...
                            <v-col cols="auto" v-if="hab.elemento">
                              <v-icon size="small" :color="getRitualColor(hab.elemento)"
                            ...
                            <v-col cols="auto" v-if="expanded" class="d-flex ml-2">
                              <v-btn
                                ...
                                size="x-small"
                                variant="text"
                                @click.stop="emit('edit:habilidade', hab)"
                              ></v-btn>
                              <v-btn
                                icon="mdi-delete"
                                size="x-small"
                                variant="text"
                                color="error"
                                @click.stop="emit('remove:habilidade', hab.instanceId)"
                              ></v-btn>
                            </v-col>
                          </v-row>
                        </template>
              ...
              <div v-else class="d-flex flex-column h-100">
                <div class="pa-2 flex-grow-0">
                  <v-row dense align="center">
                    ...
                    <v-col cols="auto">
                      <v-btn icon="mdi-plus" size="small" variant="tonal" color="primary"></v-btn>
                    </v-col>
                  </v-row>
                  <v-row dense>
                    <v-col>
                      <v-btn
                        @click="showRitualElementoFilters = !showRitualElementoFilters"
                        :prepend-icon="showRitualElementoFilters ? 'mdi-chevron-up' : 'mdi-fire'"
                        variant="tonal"
                        block
                        size="small"
                        :color="filtroRitualElemento.length > 0 ? 'primary' : undefined"
                        >Elemento</v-btn
                      >
                    </v-col>
                    <v-col>
                      <v-btn
                        @click="showRitualCirculoFilters = !showRitualCirculoFilters"
                        :prepend-icon="
                          showRitualCirculoFilters ? 'mdi-chevron-up' : 'mdi-circle-outline'
                        "
                        variant="tonal"
                        block
                        size="small"
                        :color="filtroRitualCirculo.length > 0 ? 'primary' : undefined"
                        >Círculo</v-btn
                      >
                    </v-col>
                  </v-row>
                  ...
                  <v-expand-transition>
                    <div v-if="showRitualElementoFilters" class="mt-2">
                      <v-chip-group v-model="filtroRitualElemento" show-arrows multiple>
                        <v-chip
                          v-for="filtro in opcoesFiltroRitual"
                          :key="filtro"
                          :value="filtro"
                          filter
                          size="small"
                  ...
                  <v-expand-transition>
                    <div v-if="showRitualCirculoFilters" class="mt-2">
                      <v-chip-group v-model="filtroRitualCirculo" show-arrows multiple>
                        <v-chip
                          v-for="filtro in opcoesFiltroCirculo"
                          :key="filtro"
                          :value="filtro"
                          filter
                          size="small"
                ...
                <div class="overflow-y-auto flex-grow-1">
                  <v-list lines="two" density="compact" class="pt-0">
                    <v-list-item
                      v-for="ritual in rituaisFiltrados"
                      :key="ritual.instanceId"
                      :title="ritual.nome"
                    ...
                    >
                      <template v-slot:prepend>
                        <v-avatar
                          size="32"
                          :color="getRitualColor(ritual.elemento)"
            ...
            <div class="pa-2 flex-grow-0" style="margin-top: auto">
              <v-btn
                @click="showRituais = !showRituais"
                :prepend-icon="showRituais ? 'mdi-auto-fix' : 'mdi-book-variant'"
                variant="tonal"
                block
                size="small"
              >
                {{ showRituais ? 'Voltar para Habilidades' : 'Ver Rituais' }}
              </v-btn>
            </div>
          </div>
        </v-window-item>
        ...
        <v-window-item :value="true" class="h-100">
          <div class="d-flex flex-column h-100">
            <div style="flex: 1 1 auto; overflow: hidden" class="d-flex flex-column">
              <div v-if="!showingCombate" class="d-flex flex-column h-100">
                <div class="pa-2 flex-grow-0">
                  <v-btn
                    @click="showItemLimits = !showItemLimits"
                    :prepend-icon="showItemLimits ? 'mdi-chevron-up' : 'mdi-chevron-down'"
                    variant="tonal"
                    block
                    size="small"
                    class="mb-2"
                    :color="showItemLimits ? 'primary' : undefined"
                    >Limites & Prestígio</v-btn
                  >
                    ...
                    <div v-if="showItemLimits">
                      <v-sheet border rounded class="pa-2">
                        <v-row dense>
                          <v-col cols="6"
                            ><v-text-field
                              v-model.number="character.prestigio"
                              label="Prestígio"
                              type="number"
                              density="compact"
                              hide-details
                      ...
                      <v-sheet border rounded class="pa-2 my-2 text-center">
                        <v-row dense class="text-caption"
                          ><v-col>Cat</v-col><v-col>[I]</v-col><v-col>[II]</v-col
                          ><v-col>[III]</v-col><v-col>[IV]</v-col></v-row
                        >
                        ...
                        <v-row dense class="font-weight-bold">
                          ...
                          <v-col
                            v-for="(count, i) in itensPorCategoria"
                            :key="'count' + i"
                      ...
                      <v-sheet border rounded class="pa-2">
                        <v-row dense>
                          ...
                          <v-col cols="6">
                            <v-text-field
                              v-model.number="character.carga_atual"
                              label="Carga"
                              density="compact"
                              hide-details
                              type="number"
                ...
                <div class="d-flex flex-column flex-grow-1 overflow-hidden">
                  <div class="pa-2 pt-0 flex-grow-0">
                    ...
                    >
                      <v-chip
                        v-for="cat in opcoesFiltroCategoria"
                        :key="cat"
                        :value="cat"
                        filter
                        size="x-small"
                  ...
                  <div class="overflow-y-auto flex-grow-1">
                    <v-expansion-panels variant="accordion" multiple>
                      <v-expansion-panel v-for="item in inventarioFiltrado" :key="item.instanceId">
                        <v-expansion-panel-title class="py-2" style="min-height: 48px">
                          <template v-slot:default="{ expanded }">
                            <v-row no-gutters align="center">
                              <v-col class="text-truncate font-weight-bold text-body-2">
          ...
          <v-btn 
            ...
            variant="text" 
            color="error" 
            @click.stop="removerItem(item.instanceId)"
          ></v-btn>
        </v-col>
                            </v-row>
                          </template>
  ...
  <div class="text-caption text-medium-emphasis mb-2 pa-3 pb-0" v-if="item.descricao">
    {{ item.descricao }}
  </div>
  ...
  <v-row dense class="text-caption px-3">
    <v-col cols="4" class="text-center border-e">
      <div class="text-grey-darken-1 font-weight-bold" style="font-size: 0.7rem;">TIPO</div>
      <div>{{ item.tipo }}</div>
    </v-col>
    <v-col cols="4" class="text-center border-e">
      <div class="text-grey-darken-1 font-weight-bold" style="font-size: 0.7rem;">ESPAÇO</div>
      <div>{{ item.espacos }}</div>
    </v-col>
    <v-col cols="4" class="text-center">
      <div class="text-grey-darken-1 font-weight-bold" style="font-size: 0.7rem;">CATEGORIA</div>
    ...
    <v-col cols="12" v-if="item.tipo === 'Arma'">
      <v-sheet rounded class="pa-2  border-t">
        <v-row dense>
          <v-col cols="6">
            <span class="font-weight-bold text-red-darken-4">Dano:</span> {{ item.dano }}
          </v-col>
          <v-col cols="6">
            <span class="font-weight-bold text-red-darken-4">Tipo:</span> {{ item.tipo_dano }}
          </v-col>
          <v-col cols="6">
            <span class="font-weight-bold text-red-darken-4">Crítico:</span> {{ item.critico }}
          </v-col>
          <v-col cols="6">
            <span class="font-weight-bold text-red-darken-4">Alcance:</span> {{ item.alcance }}
          </v-col>
          <v-col cols="12" class="border-t pt-1 text-center">
            <span class="font-weight-bold text-caption">Teste:</span> {{ item.periciaAtaque }} 
            <span v-if="item.atributoAtaque" class="text-grey">({{ item.atributoAtaque }})</span>
          </v-col>
        </v-row>
      </v-sheet>
    </v-col>
      ...
      <v-sheet rounded class="pa-2 border-t">
        <v-row dense>
          <v-col cols="6" class="text-center">
            <span class="font-weight-bold text-blue-darken-4">Defesa</span>
            <div class="text-h6 font-weight-bold">+{{ item.defesa }}</div>
          </v-col>
          <v-col cols="6" class="text-center">
            <span class="font-weight-bold text-blue-darken-4">Resistência</span>
            <div class="text-h6 font-weight-bold">{{ item.resistencia_dano }}</div>
          </v-col>
        </v-row>
      </v-sheet>
    </v-col>
                  ...
                  <v-window v-model="combateTab" class="h-100">
                    <v-window-item value="armas">
                      ...
                      <v-list lines="two" density="compact" bg-color="transparent">
                        <v-list-item
                          v-for="protecao in protecoesEquipadas"
                          :key="protecao.id"
                          :title="protecao.nome"
                        >
                          <template v-slot:subtitle>
                          ...
                          <template v-slot:append
                            ><v-chip size="small">Cat {{ protecao.categoria }}</v-chip></template
                          >
                        </v-list-item>
                      </v-list>
                    </v-window-item>
            ...
            <div class="pa-2 flex-grow-0" style="margin-top: auto">
              <v-btn
                @click="showingCombate = !showingCombate"
                :prepend-icon="showingCombate ? 'mdi-bag-personal' : 'mdi-sword'"
                variant="tonal"
                block
                size="small"
              >
...
const abrirEdicao = (item: ItemInstancia) => {
  // Como agora está tipado, não precisa do 'as ItemInstancia'
...
// Estado Principal
const showInv = ref(false) // false = Magia, true = Inventário
...
const showAddItemModal = ref(false) // Controle do Modal
</script>
